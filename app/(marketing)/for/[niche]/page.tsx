  ...
  return (
    <main style={{ background: "#fff", color: "#111", fontFamily: sans, fontWeight: 400, lineHeight: 1.7 }}>
      ...
      <div style={{ padding: "72px 40px 56px", maxWidth: 860, margin: "0 auto", background: "radial-gradient(ellipse 70% 50% at 50% 0%, rgba(184,134,11,0.06) 0%, transparent 70%)" }}>
        ...
        <p style={{ fontSize: 20, color: "var(--muted)", lineHeight: 1.7, marginBottom: 12, fontStyle: "italic" }}>{niche.subhead}</p>
        <p style={{ fontSize: 16, color: "var(--muted)", lineHeight: 1.85, maxWidth: 640, marginBottom: 36 }}>{niche.lede}</p>
        <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
          ...
          <Link href="/guide" style={{ fontFamily: mono, fontSize: 11, letterSpacing: "0.15em", textTransform: "uppercase", color: "var(--muted)", background: "#f5f5f5", padding: "14px 28px", borderRadius: 3, textDecoration: "none" }}>
            See how it works
          </Link>
        </div>
      </div>
      ...
      <div style={{ background: "#FFF8F0", borderTop: "1px solid #f0e4c0", borderBottom: "1px solid #f0e4c0", padding: "40px", margin: "0" }}>
        <div style={{ maxWidth: 860, margin: "0 auto" }}>
          ...
          <p style={{ fontSize: 16, color: "var(--muted)", lineHeight: 1.8, maxWidth: 680 }}>{niche.painDetail}</p>
        </div>
      </div>

      {/* Features */}
      <div style={{ padding: "60px 40px", maxWidth: 860, margin: "0 auto" }}>
        <p style={{ fontFamily: mono, fontSize: 10, letterSpacing: "0.2em", textTransform: "uppercase", color: "var(--muted)", marginBottom: 14 }}>What you can do on Spotlightly</p>
      ...
      <div style={{ background: "#FAFAF8", borderTop: "1px solid #eee", borderBottom: "1px solid #eee", padding: "56px 40px" }}>
        <div style={{ maxWidth: 860, margin: "0 auto" }}>
          <p style={{ fontFamily: mono, fontSize: 10, letterSpacing: "0.2em", textTransform: "uppercase", color: "var(--muted)", marginBottom: 14 }}>The math</p>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginTop: 28 }}>
            <div style={{ background: "#FFF5F5", border: "1px solid #f0c0c0", borderLeft: "4px solid #cc3333", padding: "24px 28px" }}>
              ...
              <p style={{ fontSize: 13, color: "var(--muted)", lineHeight: 1.6 }}>{niche.earningExample.them.take} in platform cuts and fees</p>
            </div>
            <div style={{ background: "#F5FFF8", border: "1px solid #b0ddc0", borderLeft: "4px solid #1a7a47", padding: "24px 28px" }}>
              ...
              <p style={{ fontSize: 13, color: "var(--muted)", lineHeight: 1.6 }}>{niche.earningExample.us.fee} flat — you keep everything else</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div style={{ padding: "60px 40px", maxWidth: 860, margin: "0 auto" }}>
        <p style={{ fontFamily: mono, fontSize: 10, letterSpacing: "0.2em", textTransform: "uppercase", color: "var(--muted)", marginBottom: 14 }}>Getting started</p>
          ...
          {[
            ...
            <div key={step.num} style={{ display: "grid", gridTemplateColumns: "64px 1fr", background: "#fafafa", border: "1px solid #eee", overflow: "hidden" }}>
              ...
              <div style={{ padding: "22px 28px" }}>
                ...
                <p style={{ fontSize: 14, color: "var(--muted)", lineHeight: 1.7, margin: 0 }}>{step.desc}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      <div style={{ background: "#FAFAF8", borderTop: "1px solid #eee", padding: "48px 40px" }}>
        <div style={{ maxWidth: 860, margin: "0 auto" }}>
          <p style={{ fontFamily: mono, fontSize: 10, letterSpacing: "0.18em", textTransform: "uppercase", color: "var(--muted)", marginBottom: 20 }}>Spotlightly is also built for</p>
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            {getAllNicheSlugs().filter(s => s !== niche.slug).map(s => {
              const n = getNiche(s)!;
              return (
                <Link key={s} href={`/for/${s}`} style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 16px", background: "#fff", border: "1px solid #e8e8e8", borderRadius: 3, textDecoration: "none", color: "var(--muted)", fontSize: 13, fontWeight: 500 }}>
      ...
      <div style={{ padding: "72px 40px", textAlign: "center", background: "#fff" }}>
        ...
        <p style={{ fontSize: 16, color: "var(--muted)", marginBottom: 32 }}>30-day free trial. No card required. Your handle, your page, your money.</p>
