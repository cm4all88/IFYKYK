...
export async function POST(req: NextRequest) {
  ...
  if (!post.creator?.stripe_account_id) return NextResponse.json({ error: "Creator not connected to Stripe" }, { status: 503 });
  ...
  const params = new URLSearchParams({
    ...
    "line_items[0][price_data][product_data][name]": `Unlock post · @${post.creator.handle}`,
    "line_items[0][price_data][product_data][description]": (description || "Exclusive post").slice(0, 255),
