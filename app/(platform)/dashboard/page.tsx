...
function OverviewPane({
  ...
  async function studioCommit(payload: StudioPayload): Promise<string | null> {
    const profilePayload: any = {
      bio: payload.bio.trim() || null,
      free_tier_name: payload.freeTier.name.trim() || null,
      free_tier_blurb: payload.freeTier.blurb.trim() || null,
      free_tier_perks: payload.freeTier.perks.map((s) => s.trim()).filter(Boolean),
    };
    const { error: pe } = await (supabase as any).from("creator_profiles").update(profilePayload).eq("id", profile.id);
    if (pe) { await (supabase as any).from("creator_profiles").update({ bio: profilePayload.bio }).eq("id", profile.id); }
    const { count } = await (supabase as any).from("subscription_tiers").select("id", { count: "exact", head: true }).eq("creator_profile_id", profile.id);
    const tierRows = payload.tiers
      .filter((t) => t.name.trim() && Number(t.price_monthly) > 0)
      .map((t, idx) => ({ creator_profile_id: profile.id, name: t.name.trim(), description: t.description.trim() || null, price_monthly: Number(t.price_monthly), price_yearly: t.price_yearly != null && Number(t.price_yearly) > 0 ? Number(t.price_yearly) : null, perks: t.perks.map((x) => x.trim()).filter(Boolean), color: "#F0B429", sort_order: (count ?? 0) + idx }));
    if (tierRows.length) { const { error: te } = await (supabase as any).from("subscription_tiers").insert(tierRows); if (te) return "Tiers failed to save: " + te.message; }
    if (payload.includeCampaign && payload.campaign.title.trim() && payload.campaign.goal > 0) {
      const { data: camp, error: ce } = await (supabase as any).from("campaigns").insert({ creator_profile_id: profile.id, title: payload.campaign.title.trim(), description: payload.campaign.description.trim() || null, goal_amount: payload.campaign.goal, category: payload.campaign.category, status: "active" }).select().single();
      if (ce) return "Campaign failed to save: " + ce.message;
      if (camp) {
        const rows = payload.campaign.tiers.filter((t) => t.title.trim() && Number(t.amount) > 0).map((t, idx) => ({ campaign_id: camp.id, title: t.title.trim(), amount: Number(t.amount), description: t.description.trim() || null, rewards: t.rewards.filter((r) => r.label.trim()), sort_order: idx }));
    ...
    <div className="pane">
      ...
      {studioOpen && (
        <StudioSetup
          ...
          onCommit={studioCommit}
          onClose={() => setStudioOpen(false)}
          onDone={() => { setStudioOpen(false); onSetPane("profile"); }}
        />
      )}
      ...
      <div className="pane-head pane-head--row">
        <div>
          ...
          <p style={{ fontSize:13, color:"var(--muted)", marginTop:"var(--s-2)", maxWidth:480 }}>
            PDFs, photo sets, preset packs, beat packs, templates, courses. Anything digital. Spotlightly takes 0%, you keep your full price, and the fan covers the card fee. They get an instant download link after purchase.
          </p>
        </div>
        ...
        <form onSubmit={createProduct} style={{ background:"var(--surface)", border:"1px solid var(--border)", borderRadius:"var(--r-3)", padding:"var(--s-6) var(--s-7)", marginBottom:"var(--s-4)" }}>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"var(--s-4)", marginBottom:"var(--s-4)" }}>
            ...
            <div className="form-field">
              ...
              <input ref={fileRef} type="file" style={{ display:"none" }} onChange={uploadFile} accept=".pdf,.zip,.rar,.7z,.mp3,.wav,.m4a,.mp4,.mov,.epub,.psd,.jpg,.jpeg,.png,.heic,.webp,.tif,.tiff" />
              {fileUrl ? (
          ...
          {price && (
            <div style={{ background:"rgba(240,180,41,0.06)", border:"1px solid rgba(240,180,41,0.15)", borderRadius:"var(--r-2)", padding:"var(--s-3) var(--s-4)", marginBottom:"var(--s-5)", fontSize:13 }}>
              You earn <strong style={{ color:"var(--text)" }}>${parseFloat(price||"0").toFixed(2)}</strong>
