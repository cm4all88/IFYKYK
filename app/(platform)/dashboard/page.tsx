...
// ──────────────────────────────────────────────────────────────────
function StudioSetup({ profile, onClose, onDone }: { profile: Profile; onClose: () => void; onDone: () => void }) {
  const supabase = createClient();
  ...
  async function generate() {
    ...
    try {
      const res = await fetch("/api/studio/build", {
        ...
        body: JSON.stringify({ niche, makes, fans, workingToward, displayName: profile.display_name, handle: profile.handle }),
      });
  ...
  async function commit() {
    ...
    const profilePayload: any = {
      bio: bio.trim() || null,
      free_tier_name: freeName.trim() || null,
      free_tier_blurb: freeBlurb.trim() || null,
      free_tier_perks: freePerks.split("\n").map((s) => s.trim()).filter(Boolean),
    };
    const { error: pe } = await (supabase as any).from("creator_profiles").update(profilePayload).eq("id", profile.id);
    if (pe) {
      // free tier columns may not exist yet; still save the bio so nothing is lost
      await (supabase as any).from("creator_profiles").update({ bio: profilePayload.bio }).eq("id", profile.id);
    }
    ...
    const tierRows = tiers
      ...
      .map((t, idx) => ({
        creator_profile_id: profile.id, name: t.name.trim(), description: t.description.trim() || null,
        price_monthly: Number(t.price_monthly), price_yearly: t.price_yearly != null && Number(t.price_yearly) > 0 ? Number(t.price_yearly) : null,
        perks: t.perks.map((p) => p.trim()).filter(Boolean), color: "#F0B429", sort_order: (count ?? 0) + idx,
      }));
    if (tierRows.length) {
      const { error: te } = await (supabase as any).from("subscription_tiers").insert(tierRows);
      if (te) { setSaveErr("Tiers failed to save: " + te.message); setSaving(false); return; }
    }

    if (includeCampaign && campTitle.trim() && Number(campGoal) > 0) {
      const { data: camp, error: ce } = await (supabase as any).from("campaigns").insert({
        creator_profile_id: profile.id, title: campTitle.trim(), description: campDesc.trim() || null,
        goal_amount: parseFloat(campGoal), category: campCategory, status: "active",
      }).select().single();
      if (ce) { setSaveErr("Campaign failed to save: " + ce.message); setSaving(false); return; }
      if (camp) {
        const rows = campTiers
          .filter((t) => t.title.trim() && Number(t.amount) > 0)
          .map((t, idx) => ({ campaign_id: camp.id, title: t.title.trim(), amount: Number(t.amount), description: t.description.trim() || null, rewards: t.rewards.filter((r) => r.label.trim()), sort_order: idx }));
