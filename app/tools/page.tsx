    ...
    <>
      ...
      <main style={{ maxWidth: 960, margin: "0 auto", padding: "60px 24px 120px" }}>
        ...
        <div style={{ marginBottom: 56 }}>
          <p style={{ fontFamily: "monospace", fontSize: 10, letterSpacing: ".2em", textTransform: "uppercase", color: "rgba(255,255,255,0.3)", marginBottom: 8 }}>Creator software toolkit</p>
          ...
          <p style={{ fontSize: 16, color: "rgba(255,255,255,0.5)", lineHeight: 1.75, maxWidth: 560 }}>
          ...
          {TOOL_SECTIONS.map(section => (
            <div key={section.id}>
              <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 20 }}>
                ...
                <div>
                  <h2 style={{ fontFamily: "Georgia,serif", fontSize: 26, fontWeight: 300, color: "#fff", lineHeight: 1 }}>{section.name}</h2>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(420px, 1fr))", gap: 3 }}>
                {section.tools.map((tool, i) => (
                  <a key={i} href={tool.affiliate} target="_blank" rel="noopener noreferrer nofollow"
                    style={{ display: "block", background: "#111115", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 10, padding: "24px", textDecoration: "none" }}>
                    <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 12, marginBottom: 10 }}>
                      <div>
                        <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4, flexWrap: "wrap" }}>
                          <span style={{ fontSize: 15, fontWeight: 700, color: "#F2F2F0" }}>{tool.name}</span>
                          {tool.badge && (
                            <span style={{ fontFamily: "monospace", fontSize: 9, letterSpacing: ".1em", textTransform: "uppercase", color: tool.badgeColor, background: `${tool.badgeColor}15`, border: `1px solid ${tool.badgeColor}30`, padding: "2px 8px", borderRadius: 3 }}>
                              {tool.badge}
                            </span>
                          )}
                        </div>
                      ...
                      <span style={{ fontFamily: "monospace", fontSize: 13, color: "#F0B429", fontWeight: 700, flexShrink: 0 }}>{tool.price}</span>
                    </div>
                    <p style={{ fontSize: 13, color: "rgba(255,255,255,0.5)", lineHeight: 1.65 }}>{tool.why}</p>
                    <p style={{ fontFamily: "monospace", fontSize: 10, letterSpacing: ".1em", textTransform: "uppercase", color: "rgba(255,255,255,0.2)", marginTop: 12 }}>Get it →</p>
                  </a>
                ))}
              </div>
            </div>
        ...
        <div style={{ marginTop: 80, textAlign: "center" }}>
          ...
          <a href="/gear" style={{ display: "inline-block", background: "transparent", color: "#F0B429", fontWeight: 700, fontSize: 13, padding: "12px 28px", borderRadius: 999, textDecoration: "none", border: "1px solid rgba(240,180,41,0.3)" }}>
            See the gear guide →
          </a>
        </div>
