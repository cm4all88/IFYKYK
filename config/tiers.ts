...
export function getTierForSubCount(subCount: number) {
  return CREATOR_TIERS.find(t => t.maxSubs === null || subCount <= t.maxSubs) ?? CREATOR_TIERS[CREATOR_TIERS.length - 1];
}
...
// Young creator commission tiers (no monthly fee)
export const YOUNG_CREATOR_COMMISSIONS = [
  { minEarnings: 0,    maxEarnings: 100,  rate: 0.10, capDollars: 10  },
  { minEarnings: 101,  maxEarnings: 500,  rate: 0.08, capDollars: 32  },
  { minEarnings: 501,  maxEarnings: 1000, rate: 0.06, capDollars: 30  },
  { minEarnings: 1001, maxEarnings: 3000, rate: 0.04, capDollars: 80  },
  { minEarnings: 3001, maxEarnings: 7500, rate: 0.035, capDollars: 157 },
] as const;

export function getYoungCreatorCommission(monthlyEarnings: number) {
  const tier = YOUNG_CREATOR_COMMISSIONS.find(
    t => monthlyEarnings >= t.minEarnings && monthlyEarnings <= t.maxEarnings
  );
  if (!tier) return monthlyEarnings * 0.035;
  return Math.min(monthlyEarnings * tier.rate, tier.capDollars);
}
