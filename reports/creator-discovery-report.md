...
**Run date:** 2026-07-25
**Output:** `data/creator-prospects-discovered.csv` — 85 verified creators
...
**85 creators verified. 21 emailable, 23 with a published business contact method. 0 imported. 0 contacted.**
...
The list stops at 85 rather than 100 because that is how many met the verification
...
bounds. So the `followers` column is **empty on all 85 rows by design** rather than
...
|---|---|
| Patreon | 47 |
| Own website or Bandcamp (`other`) | 38 |
...
method*. Twenty-three do: twenty-one by email, and two by a business enquiry form —
...
twenty-one with addresses are actionable inside the system today. The other two are
...
| 70–79 | 32 | Good — proven paid base, active |
| 60–69 | 25 | Moderate — monetising, weaker signal or slower cadence |
...
Thirty candidates evaluated and excluded.
...
email populated: 21 (all syntactically valid)
by platform: patreon 47, other 38
```
...
1. **Email sourcing is the bottleneck, but it is yielding.** 21 of 85 are emailable, up from 3, after finding that Bandcamp profiles and creator-owned domains both publish addresses. Both seams are still far from exhausted. The creator-owned-website
